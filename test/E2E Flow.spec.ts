...
test('Create Enquiry – Tenant Enquiry', async ({ page }) => {
  ...
  const negoPage = new NegotiationPage(page);
...
//   await enquiryPage.openRecordView('Opportunity__c', 'a0E9I000002fEXJUA2');
  ...
  await opportunityPage.verifyStages('Site Visit');
  await negoPage.selectNego(data.DocumentName);
  await negoPage.verifyFilesNego(data.DocumentName);
  await opportunityPage.verifyStages('Negotiation');
